...
### Deeply unconvinced by the ensembl-refseq stuff. Maybe just ignore this for now
...
attributes <- c('ensembl_transcript_id', 'description', 'ensembl_gene_id', 'transcript_start', 'transcript_end', 'strand', 'chromosome_name','refseq_peptide')

# only works with more recent versions'refseq_peptide_predicted')


m <- dbDriver("SQLite")
con <- dbConnect(m, dbname = "MusMusculus_mm8_TSS_RefSeq.db")

for(chr in chrs){

  cat("Generating for chr",chr);

  values<-list(chr, 'KNOWN', TRUE)
  res <- getBM(attributes, filters=c(filters,'with_refseq_peptide'), values=values, mart=ensembl)

  
#only works with more recent versions of biomart
#  res<-rbind(res,getBM(attributes, filters=c(filters,'with_refseq_peptide_predicted'), values=values, mart=ensembl))
#
...
#  res <- res[!duplicated(res),]
  ...
  res<-data.frame('EnsemblGeneID'=res$'ensembl_gene_id', 'EnsemblTranscriptID'=res$'ensembl_transcript_id', 'Description'=res$description, 'TSS'=as.integer(tss), 'Chr' = res$'chromosome_name', 'Strand'=res$strand, 'TranscriptStart'=res$'transcript_start', 'TranscriptEnd' = res$'transcript_end', 'RefSeqPeptide'=res$'refseq_peptide', 'RefSeqPredictedPeptide'=res$'refseq_peptide_predicted');
