    ...
    AnnotationData) 
{
    ...
    r2 = cbind(rownames(TSS.ordered), start(TSS.ordered), end(TSS.ordered), 
        TSS.ordered$strand)
    ...
    r1 = do.call(rbind, lapply(seq_len(numberOfChromosome), function(i) {
        ...
        if (chr %in% allChr.Anno) {
            ...
            featureRanges = IRanges(start = featureStart, end = featureStart, 
                names = featureID)
            nearestFeature = featureRanges[nearest(peakRanges, 
                featureRanges)]
            data.frame(name = name, chr = rep(chr, length(peakStart)), 
                peakStart = peakStart, peakEnd = peakEnd, feature_id = names(nearestFeature))
        }
    }))
    ...
    distancetoFeature = as.numeric(as.character(r11$peakStart)) - 
    ...
    insideFeature = (distancetoFeature >= 0 & distancetoFeature <= 
        length)
    r11$insideFeature = insideFeature
    r11$distancetoFeature = distancetoFeature
    distancetoFeature = as.numeric(as.character(r22$end_position)) - 
        as.numeric(as.character(r22$peakStart))
    ...
    RangedData(IRanges(start = c(as.numeric(as.character(r11$peakStart)), 
        as.numeric(as.character(r22$peakStart)), as.numeric(as.character(r33$peakStart))), 
        end = c(as.numeric(as.character(r11$peakEnd)), as.numeric(as.character(r22$peakEnd)), 
            as.numeric(as.character(r33$peakEnd))), names = c(as.character(r11$name), 
            as.character(r22$name), as.character(r33$name))), 
        strand = c(as.numeric(as.character(r11$strand)), as.numeric(as.character(r22$strand)), 
            r33$strand), feature = c(as.character(r11$feature_id), 
            as.character(r22$feature_id), r33$feature_id), start_position = c(as.numeric(as.character(r11$start_position)), 
            as.numeric(as.character(r22$start_position)), r33$start_position), 
        end_position = c(as.numeric(as.character(r11$end_position)), 
            as.numeric(as.character(r22$end_position)), r33$end_position), 
        insideFeature = c(unlist(r11$insideFeature), unlist(r22$insideFeature), 
            unlist(r33$insideFeature)), distancetoFeature = c(unlist(as.numeric(as.character(r11$distancetoFeature))), 
            unlist(as.numeric(as.character(r22$distancetoFeature))), 
            unlist(r33$distancetoFeature)), space = c(as.character(r11$chr), 
            as.character(r22$chr), as.character(r33$chr)))
}
