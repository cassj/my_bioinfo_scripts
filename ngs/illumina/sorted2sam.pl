...
#flag to say if you're using old (ASCII(Qsolexa+64)) scores. 
#defaults to ASCII(Qphred+64)
getopts('o', $opts);
...
sub export2sam_aux {
  ...
  if (defined $opts->{o}){
     $s->[10] =~ s/(.)/$ct->[ord($1)]/eg; # change coding from Qsolexa+64
  }else{
     $s->[10] =~ s/(.)/chr(ord($1)-31)/eg; # change coding Qphred+64
  }
  # cigar
  $s->[5] = length($s->[9]) . "M";
  # coor
  ...
  $s->[4] = 0;
  $s->[4] = $t[15] if ($t[15] ne '');
  $s->[4] = $t[16] if ($t[16] && $t[16] ne '' && $s->[4] < $t[16]);
  # mate coordinate
  $s->[6] = '*'; $s->[7] = $s->[8] = 0;
  # aux
